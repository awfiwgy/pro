...
#www.88dm.fans
    def init(self, extend=""):
        self.siteUrl = extend if extend else "https://www.88dm.tv"
        self.configCache = {}
    ...
    def detailContent(self, ids):
        ...
        for i in range(len(circuits)):
            if i >= len(sources):
                continue  # 避免索引越界
            name = circuits[i].xpath('./text()')[0].strip()
            # 线路名称保留原始名称，可根据需要自定义格式
            vod_play_from.append(name)
            # 提取当前线路下所有剧集
            eps = [f"{link.xpath('./text()')[0].strip()}${link.xpath('./@href')[0]}" for link in sources[i].xpath('.//a') if link.xpath('./text()') and link.xpath('./@href')]
            if eps:  # 确保有有效剧集才添加
